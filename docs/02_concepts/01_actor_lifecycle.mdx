...
description: Understanding Actor lifecycle, initialization, and the Apify platform
---
...
- A user aborts the Actor run with the "abort gracefully" option (`aborting` event)
- The platform needs to migrate your Actor to a different server (`migrating` event)
...
// Your Actor logic here - when the platform signals abort or migration,
// Actor.exit() will be called automatically

await Actor.exit();
```

When `gracefulShutdown` is enabled, the SDK will automatically call `Actor.exit()` when an `aborting` or `migrating` event is received. Since `Actor.exit()` waits for all event handlers to complete, you can use event handlers to perform cleanup:
...
You can also add a delay before `Actor.exit()` is called using `gracefulShutdownDelayMillis`:

```js
await Actor.init({
    gracefulShutdown: true,
    gracefulShutdownDelayMillis: 5000, // Wait 5 seconds before exiting
});
```
