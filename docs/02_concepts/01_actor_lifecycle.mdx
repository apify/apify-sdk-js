...
// - on 'aborting' event: Actor.exit() is called to terminate the run
// - on 'migrating' event: Actor.reboot() is called to continue on a new worker
...
// Register cleanup handler — runs on both exit and reboot
...
You can also add a delay before the shutdown action is triggered using `gracefulShutdownDelayMillis`:
