...
convenient [request](./02_request_storage) and [result](./03_result_storage) storages, [proxies](./04_proxy_management),
...
You can also develop your actor in an online code editor directly on the platform (you'll need an Apify Account). Let's go to the [Actors](https://console.apify.com/actors) page in the app, click _Create new_ and then go to the _Source_ tab and start writing the code or paste one of the examples from the [Examples](../03_guides) section.
