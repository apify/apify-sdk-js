...
`APIFY_LOCAL_STORAGE_DIR` environment variable, or on the [Apify platform](./01_actor_lifecycle) under the user account
...
`APIFY_LOCAL_STORAGE_DIR` to `./storage` in the current working directory and prints a warning.
...
To see a real-world example of how to get the input from the key-value store, see the [Screenshots](../03_guides/puppeteer_capture_screenshot) example.
...
When the dataset is stored on the [Apify platform](./01_actor_lifecycle), you can export its data to the following formats: HTML,
...
To see how to use the dataset to store crawler results, see the [Cheerio Crawler](../03_guides/cheerio_crawler) example.
