...
`APIFY_LOCAL_STORAGE_DIR` environment variable, or on the [Apify platform](./actor-lifecycle) under the user account
...
To see a real-world example of how to get the input from the key-value store, see the [Screenshots](../guides/capture-screenshot) example.
...
When the dataset is stored on the [Apify platform](./actor-lifecycle), you can export its data to the following formats: HTML,
...
To see how to use the dataset to store crawler results, see the [Cheerio Crawler](../guides/cheerio-crawler) example.
