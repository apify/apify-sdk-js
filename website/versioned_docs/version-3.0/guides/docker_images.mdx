...
This is the smallest image we have based on Alpine Linux. It does not include any browsers. It benefits from lightning fast builds and container startups. Headless browsers will **NOT** work with this image.
...
This image includes Puppeteer (Chromium) and the Chrome browser. It can be used with the Puppeteer library, but **NOT** with Playwright. The image supports XVFB by default, so you can run both `headless` and `headful` browsers with it.
...
Similar to [`actor-node-puppeteer-chrome`](#actor-node-puppeteer-chrome), but for Playwright. You can run Playwright, but **NOT** Puppeteer.

It uses the [`PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD`](https://playwright.dev/docs/api/environment-variables/) environment variable to block installation of more browsers into the image to keep it small. If you want more browsers, either use the [`actor-node-playwright`](#actor-node-playwright) image or override this env var.
