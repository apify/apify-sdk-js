...
export class CrawlerSetup implements CrawlerSetupOptions {
    ...
    constructor(input: Input) {
        ...
        if (this.input.useChrome && this.input.launcher !== 'chromium') {
            throw new Error(
                'useChrome option could only be used with Chromium browser selected.',
            );
        }
        ...
        if (
            !/^(domcontentloaded|load|networkidle)$/.test(this.input.waitUntil)
        ) {
            throw new Error(
                'Navigation wait until event must be valid. See tooltip.',
            );
        }
    ...
    private _createNavigationHooks(options: PlaywrightCrawlerOptions) {
        options.preNavigationHooks!.push(
            async ({ request, page, session, blockRequests }, gotoOptions) => {
                ...
                if (gotoOptions) {
                    gotoOptions.timeout =
                        (this.devtools
                            ? DEVTOOLS_TIMEOUT_SECS
                            : this.input.pageLoadTimeoutSecs) * 1000;
    ...
    private async _requestHandler(crawlingContext: PlaywrightCrawlingContext) {
        ...
        Object.defineProperties(
            ...
            Object.getOwnPropertyDescriptors(crawlingContext),
        );
