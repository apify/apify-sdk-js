...
export class Dataset extends OriginalDataset {
    ...
    public async forEachParallel(func: (item: DatasetItem) => void, options: any = {}) {
        const { parallelLoads = 20, batchSize = 50000 } = options;

        const { itemCount } = await this.getInfo() ?? { itemCount: 0 };

        return waitForCompletion(
            [...new Array(Math.ceil(itemCount / batchSize))]
                .map((_, i) => () => this.forEach(func, { limit: batchSize, offset: batchSize * i })),
            parallelLoads);
    }
    ...
    //     */
    //     public static async getDataParallel(
    //         datasetIds: string[],
    ...
    //         await concurrentWait(requestInfoArr.map((requestInfoObj) => async () => {
