 ...
 */
function withApifyEnv<T extends z.ZodType>(base: ConfigField<T>, apifyEnvVars: string | string[]): ConfigField<T>;
...
function withApifyEnv<T extends z.ZodType>(
    ...
    return field((schema ?? base.schema) as T, [...[apifyEnvVars].flat(), ...crawleeVars]);
}
...
export const apifyConfigFields = {
    ...
    standbyPort: field(
        coerceNumber.default(+LOCAL_ACTOR_ENV_VARS[ACTOR_ENV_VARS.STANDBY_PORT]),
    ...
    maxTotalChargeUsd: field(
        coerceNumber.transform((val: number) => (val === 0 ? Infinity : val)).default(Infinity),
...
export class Configuration extends CoreConfiguration {
    ...
    protected static override fields: Record<string, ConfigField> = apifyConfigFields;
